...
void UserAttitudeControl::Run()
{
	...
	// Read COM offset from mavros_gs and pass to attitude controller for coupling compensation.
	mavros_gs_s mavros_gs{};
	_mavros_gs_sub.copy(&mavros_gs);
	const Dcmf R_body_to_world{Quatf(v_att.q)};
	const Vector3f p_c_b{mavros_gs.gs[0], mavros_gs.gs[1], mavros_gs.gs[2]};
	const float total_mass = _mass_uav + _mass_manip;
	_attitude_control.setCouplingCompState(R_body_to_world, p_c_b, total_mass);
...
extern "C" __EXPORT int pregme_att_control_v1_main(int argc, char *argv[])
{
