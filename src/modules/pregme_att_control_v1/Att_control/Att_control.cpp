...
void Att_Control::setCouplingCompState(const Dcmf &R_body_to_world,
				       const Vector3f &p_c_b,
				       float total_mass)
{
					     ...
					     float pos_z)
{
	...
	// ---- COM offset angular rate coupling compensation ----
	// Delta_omega = I_b^{-1} * [ (m_total) * p_c_b x (R^T * g_n) ]
	const Vector3f g_n{0.f, 0.f, 9.8066f};
	const Vector3f body_force_dir = _R_body_to_world.transpose() * g_n;

	_delta_omega_comp = _I_b_inve * (_total_mass * (_p_c_b.cross(body_force_dir)));
	sanitizeVector3(_delta_omega_comp);
	...
	torque = 2.f * (_I_b * (Q_inv * control_vec)) + gyro_comp - _I_b * (_usr_eso.delta_esti + _delta_omega_comp);
