...
ModuleBase::Descriptor UserPositionControl::desc{task_spawn, custom_command, print_usage};
	...
	if (_parameter_update_sub.updated() || force) {
		...
		if (num_changed > 0) {
	        ...
	        _control.setControlParas(
                            ...
                            Vector3f(_param_usr_k_p_x.get(), _param_usr_k_p_y.get(), _param_usr_k_p_z.get()));


		_control.setCESOParas(
			Vector3f(_param_eso_v_l1_x.get(), _param_eso_v_l1_y.get(),_param_eso_v_l1_z.get()),
		...
		_control.setPresetTrajParas(_param_PresetTraj_l.get(), _param_PresetTraj_w.get(), _param_PresetTraj_epsilon.get(),_param_PresetTraj_k.get());
		...
		// initialize vectors from params and enforce constraints
                ...
                _takeoff.generateInitialRampValue(_param_usr_k_p_z.get());
	}
...
void UserPositionControl::Run()
{
	...
	if (_local_pos_sub.update(&local_pos)) {
		...
		// Read attitude, angular velocity, and COM offset for coupling compensation.
		vehicle_attitude_s vehicle_attitude{};
		vehicle_angular_velocity_s vehicle_angular_velocity{};
		mavros_gs_s mavros_gs{};

		_vehicle_attitude_sub.copy(&vehicle_attitude);
		_vehicle_angular_velocity_sub.copy(&vehicle_angular_velocity);
		_mavros_gs_sub.copy(&mavros_gs);

		const Dcmf R_body_to_world{Quatf(vehicle_attitude.q)};
		const Vector3f omega_body{vehicle_angular_velocity.xyz};
		const Vector3f p_c_b{mavros_gs.gs[0], mavros_gs.gs[1], mavros_gs.gs[2]};

		_control.setBodyState(R_body_to_world, omega_body);
		_control.setSystemComInBody(p_c_b);
...
// if (want_takeoff && PX4_ISFINITE(_setpoint.z)
			...
			if (!flying || _vehicle_land_detected.ground_contact) {
				_control.resetESO();
			...
			const float speed_horizontal = _param_usr_xy_vel_max.get();
		...
		if (!PX4_ISFINITE(_setpoint.position[2])
			...
			float weighting = fminf(fabsf(_setpoint.velocity[2]) / _param_usr_land_speed.get(), 1.f);
...
void UserPositionControl::failsafe(const hrt_abstime &now, trajectory_setpoint_s &setpoint,
		const PositionControlStates &states, bool warn)
{
	...
	if (_failsafe_land_hysteresis.get_state()) {
		...
		if (PX4_ISFINITE(states.velocity(0)) && PX4_ISFINITE(states.velocity(1))) {
			...
			setpoint.velocity[0] = setpoint.velocity[1] = 0.f;
			...
			setpoint.velocity[2] = _param_usr_land_speed.get();
...
void UserPositionControl::reset_setpoint_to_nan(trajectory_setpoint_s &setpoint)
{
...
extern "C" __EXPORT int pregme_pos_control_v1_main(int argc, char *argv[])
{
	return ModuleBase::main(UserPositionControl::desc, argc, argv);
}
